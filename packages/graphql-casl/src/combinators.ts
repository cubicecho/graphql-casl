    ...
    if (!isCheckableRule(operand)) {
      throw new Error(
        `graphql-casl: \`${combinator}()\` operand ${index} is not a checkable rule, so its ` +
          ...
          'or `createCan(...)`. `createCan(...).onResult`, `scopeArgs(...)` and `grants(...)` ' +
          'rules are never combinable — one needs the resolved value to decide, one decides by ' +
          'rewriting the arguments and calling the resolver itself, and one tags what the ' +
          'resolver returned. Compose those with `wrap()` instead.',
      );
    }
